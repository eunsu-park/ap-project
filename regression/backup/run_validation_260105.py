...
def info_get_12(I, S, epoch):
    config = default_config.copy()
    experiment_name = f"MULTITARGET_{I}_1-2_{S:02d}"
    ...
    config["data"]["target_days"] = [1, 2]
    config["data"]["target_start_index"] = 80
    config["data"]["target_end_index"] = 80 + 16
    ...
    output_dir = f'/home/hl545/ap/renew/results/{experiment_name}/validation/epoch_{epoch:04d}'
...
def info_get_13(I, S, epoch):
    ...
    config["data"]["target_days"] = [1, 2, 3]
    config["data"]["target_start_index"] = 80
    config["data"]["target_end_index"] = 80 + 24
    ...
    for I in INPUT_DAYS:
        for S in range(8):
            ...
            for epoch in range(20, 101, 20):
                command = info_get_12(I, S, epoch)
                if command :
                    commands.append(command)
            if len(commands) > 0 :
                job_name = f"AUTO-VALIDATION-AP_{I}"
