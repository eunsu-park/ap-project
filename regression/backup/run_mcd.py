...
def info_get(I, T, S, epoch):
    ...
    config_name = f"AUTO-MCD_{experiment_name}"
    config_path = f"./configs/{config_name}.yaml"
    config_path = os.path.abspath(config_path)
    with open(config_path, 'w') as f:
        yaml.dump(config, f)
    commands = f"{PYTHON_PATH} monte_carlo_dropout.py --config-name {config_name}"

    job_name = f"AUTO-MCD-AP_{I}_{T+1}"
