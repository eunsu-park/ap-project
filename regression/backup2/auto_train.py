...
def run(base_config, info, dry_run=True):
    ...
    print(f"\n=== Submitting training job for base experiment: {experiment_name} ===")
...
def run_under(base_config, info, dry_run=True):
    """
    언더샘플링 앙상블 실험의 training을 실행
    NUM_SUBSAMPLE개의 서브샘플을 하나의 job으로 묶어서 순차 실행
    """
    commands = []
...
def run_mix(base_config, info, dry_run=True):
    """
    오버샘플링 + 언더샘플링 혼합 실험의 training을 실행
    NUM_SUBSAMPLE개의 서브샘플을 하나의 job으로 묶어서 순차 실행
    각 서브샘플에 오버샘플링도 함께 적용
    """
    commands = []
...
if __name__ == "__main__":
    ...
    print("="*80)
    print("AUTO TRAIN - SLURM Job Generator")
    print("="*80)
