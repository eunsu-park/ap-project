...
def main():
    ...
    parser.add_argument('--input-dir', type=str, 
                       default='/Users/eunsupark/ap_project/data/sdo_jp2/aia',
                       help='Input directory path')
    parser.add_argument('--output-dir', type=str,
                       default='/Users/eunsupark/ap_project/data/processed',
    ...
    parser.add_argument('--num-sequence', type=int, default=20,
...
# # 상수 정의
# ROOT_SEARCH = "/Users/eunsupark/ap_project/data/sdo_jp2/aia"
# ROOT_DATASET = "/Users/eunsupark/ap_project/data/processed"
# WAVES = (193, 211)
# TIME_STEP = 6  # hour
# NUM_SEQUENCE = 20
...
# def main_optimized(date_target, skip_existing=True):
#     """단순하게 최적화된 날짜별 처리 함수"""
#     try:
#         # 1. 출력 디렉토리 설정 (생성하지 않고 경로만 설정)
#         date_start = date_target
#         date_end = date_start + datetime.timedelta(hours=TIME_STEP * (NUM_SEQUENCE - 1))
#         date_dir = f"{ROOT_DATASET}/{date_start:%Y%m%d%H}-{date_end:%Y%m%d%H}"
        
#         # 기존 완료 체크 (디렉토리가 존재할 경우에만)
#         if skip_existing and os.path.exists(date_dir):
#             expected_files = NUM_SEQUENCE * len(WAVES)
#             existing_files = 0
...
#                 data_path = f"{ROOT_SEARCH}/{wave:d}/{year:04d}/{year:04d}{month:02d}{day:02d}"
#                 data_name = f"{year:04d}_{month:02d}_{day:02d}__{hour:02d}_00_*_*__SDO_AIA_AIA_{wave:d}.jp2"
#                 data_list = sorted(glob(f"{data_path}/{data_name}"))
...
#             date_current += datetime.timedelta(hours=TIME_STEP)

#         # 3. 유효성 검사 (디렉토리 생성 전에 확인)
#         for wave_idx, file_list in enumerate(filelists):
#             if len(file_list) != NUM_SEQUENCE:
#                 print(f"{date_target} : Invalid file list length for wave {WAVES[wave_idx]}. Skipping.")
#                 return False
#             for i, file_path in enumerate(file_list):
#                 if not file_path or not os.path.exists(file_path):
#                     print(f"{date_target} : Missing file for wave {WAVES[wave_idx]}, sequence {i}. Skipping.")
#                     return False
...
#         # 5. 파일 처리
#         total_files = NUM_SEQUENCE * len(WAVES)
#         processed = 0
        
#         for wave_idx, (wave, file_list) in enumerate(zip(WAVES, filelists)):
#             save_dir = f"{date_dir}/{wave:d}"
...
#                 if run_optimized(file_path, save_path, skip_existing):
#                     processed += 1
...
#                 if total_files < NUM_SEQUENCE * len(WAVES) * 0.1:  # 10% 미만이면 정리
#                     import shutil
#                     shutil.rmtree(date_dir)
#                     print(f"{date_target} : Cleaned up incomplete directory")
#         except:
...
# def process_dates_simple_parallel(start_date, end_date, max_workers=None, skip_existing=True):
#     """단순하고 안정적인 병렬 처리"""
    
#     if start_date.hour % 3 != 0 :
#         start_date += datetime.timedelta(hours=3 - start_date.hour % 3)
...
#     while current_date <= end_date:
#         dates_to_process.append(current_date)
#         current_date += datetime.timedelta(hours=3) # 3시간 간격
...
#         max_workers = min(cpu_count(), len(dates_to_process))
...
#         with ProcessPoolExecutor(max_workers=max_workers) as executor:
#             # 모든 작업 제출
#             future_to_date = {executor.submit(main_optimized, date, skip_existing): date for date in dates_to_process}
...
# # 원본 호환성 함수들
# def run(file_path, save_path):
#     """원본 호환성을 위한 래퍼"""
#     return run_optimized(file_path, save_path, skip_existing=False)

# def main(date_target):
#     """원본 호환성을 위한 래퍼"""
#     return main_optimized(date_target, skip_existing=False)

# def process_dates_parallel(start_date, end_date, max_workers=None):
#     """원본 호환성을 위한 래퍼"""
#     return process_dates_simple_parallel(start_date, end_date, max_workers, skip_existing=False)
...
#     start_date = datetime.datetime(year_start, month_start, day_start, hour_start)
#     end_date = datetime.datetime(2024, 12, 31, 23)
    
#     # 테스트를 위한 작은 범위
#     # end_date = start_date + datetime.timedelta(days=2)  # 2일만 테스트
    
#     # 단순 최적화된 병렬 처리 실행
#     process_dates_simple_parallel(start_date, end_date, max_workers=6)