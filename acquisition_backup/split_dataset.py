...
# ====== 1. validation month 규칙 (새 규칙) ======
def get_val_months(year: int):
    """
    새 규칙:
    2010: 10,11,12
    2011:  1, 2, 3
    2012:  4, 5, 6
    2013:  7, 8, 9
    2014: 10,11,12 (이후 4년 주기로 반복)
    """
    r = (year - 2010) % 4
    if   r == 0: return {10, 11, 12}
    elif r == 1: return {1, 2, 3}
    elif r == 2: return {4, 5, 6}
    else:        return {7, 8, 9}

def is_validation_row(file_name: str) -> bool:
    """파일명에서 연/월 추출 후 validation 여부 판단"""
    year = int(file_name[0:4])
    month = int(file_name[4:6])
    return month in get_val_months(year)
...
# validation month 박스: (중심 x,y) = (m-1, row_idx),
# 셀 경계 기준으로는 (m-1-0.5, row_idx-0.5) 시작, width=1,height=1
ax = plt.gca()
for row_idx, year in enumerate(years):
    val_months = get_val_months(year)
    for m in val_months:
        rect = plt.Rectangle((m - 1 - 0.5, row_idx - 0.5),
                             1, 1, fill=False,
...
yearly_counts = yearly_counts.reindex(columns=["validation", "train"])
...
plt.bar(years, train_vals, label="train")
plt.bar(years, val_vals, bottom=train_vals, label="validation")
