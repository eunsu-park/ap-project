...
def process_single_timestamp(t: datetime.datetime) -> str:
    ...
    # Fetch data
    result = get_all_data_at_time(t, time_range_minutes=TIME_RANGE_MINUTES)

    # Check all channels exist
    if not all(not result[ch].empty for ch in SDO_CHANNELS):
        return f"Skip (missing): {t}"

    # OMNI to dict
    omni = result['omni'].iloc[0].to_dict() if not result['omni'].empty else {}
...
def main(num_workers: int = None):
    ...
    skipped_error = sum(1 for r in results if "error" in r)
    print(f"\nCompleted: {saved} saved, {skipped_exists} existed, {skipped_missing} missing, {skipped_error} errors")
