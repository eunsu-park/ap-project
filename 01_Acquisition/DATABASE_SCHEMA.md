...
## 3. Legacy vs New Schema

### SDO Schema Comparison

| Aspect | Legacy Schema | New Schema |
|--------|---------------|------------|
| Tables | `aia_193`, `aia_211`, `hmi_magnetogram` (개별) | `sdo` (통합) |
| Primary Key | `datetime` | `(telescope, channel, datetime)` |
| datetime_round | ✅ 있음 | ❌ 없음 |
| quality | ❌ 없음 (quality=0만 저장) | ✅ 있음 (모든 quality 저장) |
| Database | `sdo` | `solar_images` |

### Data Storage Policy

| Data Source | Legacy Policy | New Policy |
|-------------|---------------|------------|
| SDO | 정각시 기준 가장 가까운 quality=0 데이터만 저장 | **모든 유효한 데이터 저장** (quality 무관) |
| LASCO | 모든 데이터 저장 | 동일 |
| SECCHI | 모든 데이터 저장 | 동일 |
...
### 4.2 sdo_config.yaml (Legacy wrapper)

기존 스크립트 호환을 위해 유지. `database: solar_images`로 설정되어 새 스키마 사용.

```yaml
db_config:
  database: solar_images  # Points to new schema
```
...
│   │   ├── hmi/
│   │   │   └── YYYY/MM/DD/
│   │   │       └── hmi.m_45s.YYYY-MM-DDTHH_MM_SS.*.fits
...
│   └── secchi/
│       └── datatype/spacecraft/instrument/YYYY/MM/DD/
└── space_weather/
    └── omni/
        ├── low_resolution/
        └── high_resolution/
...
레거시 스키마에서 새 스키마로 마이그레이션:

```bash
# Dry-run (변경 없이 확인만)
python3 scripts/migrate_to_new_schema.py --migrate-sdo

# 실제 마이그레이션 실행
python3 scripts/migrate_to_new_schema.py --migrate-sdo --execute
```
