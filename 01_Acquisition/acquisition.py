...
def get_sdo_closest_to_time(
    ...
    with PostgresManager(**DB_CONFIG_SOLAR_IMAGES) as db:
        results = db.execute(
            query,
            params=(telescope, channel, start_time, end_time, target_time),
            fetch=True
        )
        return to_dataframe(results, parse_dates=['datetime'])
...
def get_all_data_at_time(
    ...
    Returns:
        ...
        - 'aia_193': SDO AIA 193 data (closest within range, quality=0)
        - 'aia_211': SDO AIA 211 data (closest within range, quality=0)
        - 'hmi_magnetogram': SDO HMI magnetogram data (closest within range, quality=0)
    ...
    # Fetch SDO data (closest match within time range, quality=0)
