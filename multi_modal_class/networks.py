...
def create_model(options, logger=None):
    ...
    group_size = getattr(options, 'group_size', 24)
    num_target_groups = options.target_sequence_length // group_size
    
    message = f"Enhanced MultiModalModel (Classification) with Transformer, ConvLSTM, and CrossModalFusion created. Output shape: (batch, {num_target_groups}, {options.num_target_variables})"
...
if __name__ == "__main__":
    ...
    model = create_model(options, logger=None)
    print(model)

    sdo_shape = (
        options.batch_size,
        options.convlstm_input_channels,
        options.convlstm_input_image_frames,
        options.image_size,
        options.image_size
    )
    ...
    inputs_shape = (
        options.batch_size,
        options.input_sequence_length,
        options.num_input_variables
    )

    targets_shape = (
        options.batch_size,
        1,
        options.num_target_variables
    )
