...
def make_target(targets, group_size=24, threshold=48):
    """Transform targets into binary classification based on max values in groups.
    
    Args:
        targets: Input tensor of shape (len_vector, num_vector)
        group_size: Size of each group (default: 24)
        threshold: Threshold value for binary classification (default: 48)
        
    Returns:
        Binary tensor of shape (num_groups, num_vector) where each value is 1 if 
        max value in group > threshold, else 0
    """
    len_vector, num_vector = targets.shape
    num_groups = len_vector // group_size
    
    # Reshape to group the data
    targets_reshaped = targets.view(num_groups, group_size, num_vector)
    
    # Calculate max values for each group along dim=1
    max_values, _ = targets_reshaped.max(dim=1)  # (num_groups, num_vector)
    
    # Convert to binary based on threshold
    result = (max_values > threshold).float()  # (num_groups, num_vector)
