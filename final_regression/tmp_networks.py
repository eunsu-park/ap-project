...
class ConvLSTMModel(nn.Module):
    ...
    def forward(self, x: torch.Tensor, return_sequence: bool = False) -> torch.Tensor:
        ...
        Args:
            ...
            return_sequence: If True, return full sequence instead of final hidden state only.
            
        Returns:
            If return_sequence=False: Output tensor of shape (batch, output_dim).
            If return_sequence=True: Output tensor of shape (batch, seq_len, output_dim).
        """
        if x.dim() != 5:
        ...
        # Store sequence outputs if needed
        if return_sequence:
            sequence_outputs = []
        ...
        for t in range(seq_len):
            ...
            # If return_sequence, save this timestep's output
            if return_sequence:
                final_hidden_t = hidden_states[-1][0]  # (batch, hidden_channels, height, width)
                pooled_t = self.global_pool(final_hidden_t).squeeze(-1).squeeze(-1)  # (batch, hidden_channels)
                output_t = self.output_projection(pooled_t)  # (batch, output_dim)
                sequence_outputs.append(output_t)
        
        if return_sequence:
            # Stack all timesteps: (batch, seq_len, output_dim)
            return torch.stack(sequence_outputs, dim=1)
        
        # Use final hidden state from last layer (original behavior)
...
class TransformerEncoderModel(nn.Module):
    ...
    def forward(self, x: torch.Tensor, return_sequence: bool = False) -> torch.Tensor:
        ...
        Args:
            ...
            return_sequence: If True, return full sequence instead of pooled output.
            
        Returns:
            If return_sequence=False: Encoded features of shape (batch_size, d_model).
            If return_sequence=True: Encoded sequence of shape (batch_size, seq_len, d_model).
        ...
        # Return sequence if requested (for regression with temporal output)
        if return_sequence:
    ...
    def forward(self, transformer_features: torch.Tensor, convlstm_features: torch.Tensor) -> torch.Tensor:
        ...
        Args:
            transformer_features: Features from transformer. 
                Shape: (batch, feature_dim) or (batch, seq_len, feature_dim).
            convlstm_features: Features from ConvLSTM.
                Shape: (batch, feature_dim) or (batch, seq_len, feature_dim).
            
        Returns:
            Fused features with same shape as inputs.
        """
        # Check if inputs are sequences
        is_sequence = transformer_features.dim() == 3 and convlstm_features.dim() == 3
        
        if is_sequence:
            # Handle sequence inputs for regression
            batch_size, seq_len_trans, feature_dim = transformer_features.shape
            _, seq_len_conv, _ = convlstm_features.shape
            
            # For cross-attention with different sequence lengths, use interpolation
            if seq_len_trans != seq_len_conv:
                # Interpolate convlstm to match transformer sequence length
                convlstm_features = F.interpolate(
                    convlstm_features.transpose(1, 2),  # (batch, feature_dim, seq_len_conv)
                    size=seq_len_trans,
                    mode='linear',
                    align_corners=False
                ).transpose(1, 2)  # (batch, seq_len_trans, feature_dim)
            
            # Process each timestep
            fused_sequence = []
            for t in range(seq_len_trans):
                trans_t = transformer_features[:, t, :]  # (batch, feature_dim)
                conv_t = convlstm_features[:, t, :]  # (batch, feature_dim)
                
                # Cross-attention in both directions
                transformer_attended = self.transformer_to_convlstm(trans_t, conv_t)
                convlstm_attended = self.convlstm_to_transformer(conv_t, trans_t)
                ...
                residual = (trans_t + conv_t) / 2
                output_t = self.final_norm(fused_features + residual)
                
                fused_sequence.append(output_t)
            
            # Stack all timesteps
            return torch.stack(fused_sequence, dim=1)  # (batch, seq_len, feature_dim)
        
        else:
            # Original single-vector fusion for classification
            # Cross-attention in both directions
...
class MultiModalModel(nn.Module):
    ...
    def __init__(
        ...
        output_type='classification', num_classes=2,
        ...
        # Output head - depends on output_type
        if output_type == 'classification':
            ...
            self.regression_head = nn.Sequential(
                ...
                nn.ReLU(),
                nn.Dropout(fusion_dropout),
                nn.Linear(transformer_d_model // 2, num_target_variables)
            )
            self.num_target_variables = num_target_variables
    ...
    def forward(self, solar_wind_input, image_input, return_features=False):
        ...
        Returns:
            ...
            If output_type='regression':
                Regression predictions of shape (batch, target_sequence_length, num_target_variables).
        ...
        if self.output_type == 'classification':
            # Classification mode - use pooled features
            transformer_features = self.transformer_model(solar_wind_input, return_sequence=False)
            convlstm_features = self.convlstm_model(image_input, return_sequence=False)
            ...
            # Regression mode - use sequence features
            transformer_features_seq = self.transformer_model(solar_wind_input, return_sequence=True)
            convlstm_features_seq = self.convlstm_model(image_input, return_sequence=True)
            
            # Apply cross-modal fusion on sequences
            fused_features = self.cross_modal_fusion(transformer_features_seq, convlstm_features_seq)
            # fused_features: (batch, seq_len, d_model)
            
            # Select last target_sequence_length timesteps
            last_steps = fused_features[:, -self.target_sequence_length:, :]
            # last_steps: (batch, target_sequence_length, d_model)
            
            # Apply regression head to each timestep
            batch_size, seq_len, d_model = last_steps.shape
            last_steps_flat = last_steps.reshape(batch_size * seq_len, d_model)
            output_flat = self.regression_head(last_steps_flat)
            output = output_flat.reshape(batch_size, seq_len, self.num_target_variables)
            # output: (batch, target_sequence_length, num_target_variables)
            
            # For return_features, use pooled versions
            if return_features:
                transformer_features = self.transformer_model(solar_wind_input, return_sequence=False)
                convlstm_features = self.convlstm_model(image_input, return_sequence=False)
        ...
        if return_features:
            if self.output_type == 'regression':
                # transformer_features and convlstm_features already computed above
...
def create_model(config, logger=None):
    ...
    Returns:
        MultiModalModel instance configured for classification or regression.
    """
    num_input_variables = len(config.data.input_variables)
    num_target_variables = len(config.data.target_variables)
    
    # Get output type from config (default to classification for backward compatibility)
    output_type = getattr(config.model, 'output_type', 'classification')
    ...
    if output_type == 'classification':
        ...
        num_target_groups = 1  # Not used in regression, but needed for backward compatibility
        target_sequence_length = config.data.target_sequence_length
        num_classes = 2  # Not used in regression
        message = f"Enhanced MultiModalModel (Regression) with Transformer, ConvLSTM, and CrossModalFusion created. Output shape: (batch, {target_sequence_length}, {num_target_variables})"
...
def main(config):
    ...
    if config.model.output_type == "regression" :
        targets_shape = (
            config.experiment.batch_size,
    ...
    elif config.model.output_type == "classification" :
    ...
    targets = torch.randn(targets_shape)
